...
- (void)writeDataPoint:(GosDeviceDataPoint)dataPoint value:(id)value
{
    ...
    if([self.delegate respondsToSelector:@selector(GosDeviceControl:deviceUncontrol:)]){
        [self.delegate GosDeviceControl:self deviceUncontrol:@"writeDataPoint"];
    }
}
...
- (void)writeColor:(int)colorR colorG:(int)colorG colorB:(int)colorB
{
    ...
    if ([self.delegate respondsToSelector:@selector(GosDeviceControl:deviceUncontrol:)])
    {
        [self.delegate GosDeviceControl:self deviceUncontrol:@"writeColor"];
    }
...
{
    //发送通知
    [[NSNotificationCenter defaultCenter] postNotificationName:GosDeviceControlDataValueUpdateNotification  //注册的通知名称
                                                        object:nil];    //接受通知的对象，通知不传参时可使用该方法
    
    [[NSNotificationCenter defaultCenter] postNotificationName:@"Name"      //注册的通知名称
                                                        object:nil          //接受通知的对象
                                                      userInfo:nil];        //为字典类型的数据，可以传递相关数据
}
